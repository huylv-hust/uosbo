...
class Model_Ujob
{
	...
	public function get_search_data($export = false)
	{
		...
		{
			$sql = 'select sssale_id from sssale where sale_type = :sale_type';
        ...
        if(\Fuel\Core\Input::get('is_webtoku', null) != null)
        {
            $config['where'][] = array(
                'is_webtoku',
                '=',
                \Fuel\Core\Input::get('is_webtoku'),
            );
        }
		...
		if($export)
		{
			$config_pagination['per_page'] = 100000; // get all
		}
