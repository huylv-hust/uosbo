...
Class Model_Mmedia extends \Fuel\Core\Model_Crud
{
    ...
    public function save_data($data = array())
    {
        ...
        if (isset($data['m_media_id']) == false || strlen($data['m_media_id']) == 0) {
            unset($data['m_media_id']);
    ...
    {
        $query = DB::select('m_media.*', 'm_partner.branch_name', 'm_group.name')->from('m_media');
        ...
        if (isset($filters['start_id']) && $filters['start_id'] != '') {
            if (!Utility::check_number($filters['start_id'])) {
                // if start_id is character
                $filters['start_id'] = 9999999999;
            }
            $query->where('m_media.m_media_id', '>=', $filters['start_id']);
        }

        if (isset($filters['end_id']) && $filters['end_id'] != '') {
            $query->where('m_media.m_media_id', '<=', $filters['end_id']);
        }

        if (isset($filters['m_media_id']) && $filters['m_media_id']) {
            $query->where('m_media.m_media_id', '=', $filters['m_media_id']);
        }
        ...
        if (array_key_exists('is_hidden', $filters)) {
            $query->where('m_media.is_hidden', '=', $filters['is_hidden']);
        }
        ...
        if (isset($filters['media_name']) && $filters['media_name']) {
            $query->where(\Fuel\Core\DB::expr('CONCAT(m_media.media_name, m_media.media_version_name)'), 'like', '%' . $filters['media_name'] . '%');
        }
        ...
        if (isset($filters['keyword_modal']) && $filters['keyword_modal']) {
            ...
            foreach ($arr_keyword as $k => $v) {
                $query->where(\Fuel\Core\DB::expr('CONCAT(m_group.name, m_partner.branch_name, m_media.media_name, m_media.media_version_name)'), 'like', '%' . $v . '%');
            }
    ...
    /**
     * @author HuyLv6635
     * action autocomplete keyword search medias
     * @return mixed
     */
    public function get_list_concat_media()
    {
        $query = \Fuel\Core\DB::select(\Fuel\Core\DB::expr('CONCAT(media_name, media_version_name) as media_name'))
