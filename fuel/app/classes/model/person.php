...
class Model_Person extends \Orm\Model
{
	...
	{
		$query = \Fuel\Core\DB::select('person.person_id', 'person.sssale_id', 'person.post_id', 'm_group.name', 'm_partner.branch_name', 'm_ss.ss_name', 'm_user.department_id', 'm_media.*')->from('person');
		...
		$query->or_where('person.job_id', 'is', null);
	 ...
	 * @return int
	 */
	public function count_person_in_post($filters)
	{
		$query = \Fuel\Core\DB::select('person.person_id')->from('person');
		$query->join('m_post','left')->on('m_post.post_id', '=', 'person.post_id');
		if($filters['m_media_id'] === null)
			$query->where('m_post.m_media_id', '=', -1);
		else
			$query->where('m_post.m_media_id', '=', $filters['m_media_id']);
	...
	{
		$subject = '【おしごとnavi】新しい求人応募がありました。';
		...
		$data['bo_url'] = \Fuel\Core\Config::get('uosbo_url');
		return \Utility::sendmail($mailto, $subject, $data, 'email/person_admin');
	}

	public function get_data_for_mail($sssale_id)
	{
		$select = DB::select('m_ss.ss_name','m_partner.branch_name','sssale.sale_name','m_group.name', 'm_partner.department_id')->from('sssale');
