<ul class="dropdown-menu" name="add-pulldown">
    ...
    <?php if ($row['login_info']['division_type'] == 1) {?>
	<li class="<?php echo Utility::is_allowed(MyAuth::$role_edit)?>" ><a name="add-btn" class="delete_job" style="cursor: pointer" onclick="delete_job('<?php echo $row['job_id']?>')"><i class="glyphicon glyphicon-trash"/></i> 削除</a></li>
    <?php } ?>
	...
	<li class="<?php echo Utility::is_allowed(MyAuth::$role_public)?>"><a style="cursor: pointer" onclick="is_available('<?php echo $row['job_id']?>','<?php echo (1-(int)$row['is_available'])?>')"><i class="glyphicon glyphicon-remove"></i> 掲載停止中にする</a></li>
	<?php } else { ?>
	<li class="<?php echo Utility::is_allowed(MyAuth::$role_public)?>"><a onclick="is_available('<?php echo $row['job_id']?>','<?php echo (1-(int)$row['is_available'])?>')" style="cursor: pointer"><i class="glyphicon glyphicon-ok"></i> 掲載中にする</a></li>
	<?php } ?>
