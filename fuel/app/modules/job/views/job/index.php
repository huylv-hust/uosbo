...
<form class="form-inline" id="job_form" method="post" enctype="multipart/form-data">
	...
	<table class="table table-striped">
		<tbody>
			<tr>
				<th class="text-right" width="20%">しごさが掲載期間</th>
				<td width="80%">
...
?>
			...
			<tr>
				...
				<td>
					...
					<?php echo Form::input('conscription_start_date', $conscription_start_date, array('class' => 'form-control dateform', 'size' => '12', 'type' => 'text', 'placeholder' => '開始日')); ?>
					<?php
						if($conscription_start_date != $old_data['conscription_start_date']) {
							echo '<span class="edit-before">'.$old_data['conscription_start_date'].'</span>';
						}
					?>
					～
					<?php echo Form::input('conscription_end_date', $conscription_end_date, array('class' => 'form-control dateform', 'size' => '12', 'type' => 'text', 'placeholder' => '終了日')); ?>
					<?php
					if($conscription_end_date != $old_data['conscription_end_date']) {
						echo '<span class="edit-before">'.$old_data['conscription_end_date'].'</span>';
					}
			...
			<tr>
				...
				<td>
					...
					<?php echo Form::input('pickup_start_date', $pickup_start_date, array('class' => 'form-control dateform', 'size' => '12', 'type' => 'text', 'placeholder' => '開始日')); ?>
					<?php
					if($pickup_start_date != $old_data['pickup_start_date']) {
						echo '<span class="edit-before">'.$old_data['pickup_start_date'].'</span>';
					}
					?>
					～
					<?php echo Form::input('pickup_end_date', $pickup_end_date, array('class' => 'form-control dateform', 'size' => '12', 'type' => 'text', 'placeholder' => '終了日')); ?>
					<?php
					if($pickup_end_date != $old_data['pickup_end_date']) {
						echo '<span class="edit-before">'.$old_data['pickup_end_date'].'</span>';
					}
			...
			<tr>
				...
				<td>
					<span id="ss-name" addr1="<?php echo $addr1 ?>" addr2="<?php echo $addr2 ?>" addr3="<?php echo $addr3 ?>"><?php echo $ss_id ? $ss_name_edit : '※未選択' ;?></span>
					<input type="hidden" value="<?php echo $ss_id;?>" name="ss_id" id="hidden_ss_id">
					<?php
						if($ss_id != $old_data['ss_id']) {
					?>
							<span class="edit-before">
								<?php echo $ss_name_old;?>
							</span>
					<?php
						}
					?>
					...
					<div class="input-group">
						...
						<?php
						$sssale_edit = array('' => '勤務形態を選択して下さい') + $sssale_edit;
						echo Form::select('sssale_id', $sssale_id, $sssale_edit, array('class' => 'form-control'));
						?>
					</div>
                    <span class="text-info">※必須</span>

					<?php
					if($sssale_id != $old_data['sssale_id']) {
						?>
						<span class="edit-before">
								<?php echo $sssale_name_old;?>
							</span>
						<?php
					}
					?>
...
<?php } ?>
			...
			<tr class="hide">
				<th class="text-right">時間帯</th>
				<td>
					<?php foreach (Constants::$time_targets as $k => $v) {?>
					<label class="radio-inline">
						<?php echo Form::radio('time_target',$k, (isset($time_target) && $time_target == $k) ? true : false);
							echo $v;?>
					</label>
					<?php } if (isset($is_show_old['time_target']) && isset(Constants::$time_targets[$old_data['time_target']])) {?>
					<div class="edit-before"><?php echo Constants::$time_targets[$old_data['time_target']] ?></div>
					<?php }?>
				</td>
			</tr>
			<tr>
				...
				<td>
					<?php foreach (Constants::$person_targets as $k => $v) {?>
					<label class="radio-inline">
						<?php echo Form::radio('person_target',$k, (isset($person_target) && $person_target == $k) ? true : false);
						echo $v;?>
					</label>
					<?php } if (isset($is_show_old['person_target']) && isset(Constants::$person_targets[$old_data['person_target']])) {?>
						<div class="edit-before"><?php echo Constants::$person_targets[$old_data['person_target']] ?></div>
					<?php }?>
				</td>
			</tr>
			<tr>
...
<?php if (isset($is_show_old['url_youtube'])) { ?><span class="edit-before edit-before-1"><?php echo $old_data['url_youtube'] ?></span><?php } ?>
			...
			<tr>
				<th class="text-right">勤務地の住所表示</th>
				<td>
                    ...
                    <div class="text-info">※外部顧客で、町名や番地をネット上に表示させたくない場合は「非表示」を選択してください</div>
			...
			<tr>
				...
				<td>
					<label class="radio-inline hide"><?php echo Form::radio('work_location_display_type', 1, ($work_location_display_type == 1) ? true : false); ?>カセット式</label>
...
<?php } ?>
			...
			<tr>
				...
				<td>
                    <?php echo Fuel\Core\Form::select('occupation_category', $occupation_category, Occupation::$categories, array('class' => 'form-control')); ?>
						<?php echo Fuel\Core\Form::select('occupation', $occupation, $occupations, array('class' => 'form-control')); ?>
						<?php if (isset($is_show_old['occupation'])) { ?><span class="edit-before"><?php echo isset(Occupation::$rows[$old_data['occupation']]) ? Occupation::$rows[$old_data['occupation']]['name'] : "" ?></span><?php } ?>
				</td>
			</tr>
			<tr>
...
?>
		...
		<tr>
			...
			<td>
				<textarea class="form-control" cols="80" rows="5" name="business_description" length="40"<?php echo $login_info['division_type'] != 1 ? ' readonly' : '' ?>><?php echo $business_description ?></textarea>
...
<?php } elseif ($old_data['is_apply_by_mobile'] == 2 && $is_apply_by_mobile !=2) { ?>
		...
		<tr>
			...
			<td>
				...
				$telReadOnly = $login_info['division_type'] != 1;
				?>
<?php echo Form::input('phone_name1', $phone_name1, array('class' => 'form-control', 'size' => '20', 'type' => 'text', 'placeholder' => '名称', 'length' => '20')); ?>
				...
				<?php echo Fuel\Core\Form::input('phone_number1_1', isset($_arr_phone1['0']) ? $_arr_phone1['0'] : '', array('class' => 'form-control', 'size' => 5, 'placeholder' => '例)03', 'maxlength' => '5', 'readonly' => $telReadOnly)); ?>
				<?php if($_arr_phone1_old['0'] !== $_arr_phone1['0']){?><span class="edit-before"><?php echo $_arr_phone1_old['0'] ?></span><?php } ?>
				-
<?php echo Fuel\Core\Form::input('phone_number1_2', isset($_arr_phone1['1']) ? $_arr_phone1['1'] : '', array('class' => 'form-control', 'size' => 4, 'placeholder' => '例)1234', 'maxlength' => '4', 'readonly' => $telReadOnly)); ?>
				...
				-
<?php echo Fuel\Core\Form::input('phone_number1_3', isset($_arr_phone1['2']) ? $_arr_phone1['2'] : '', array('class' => 'form-control', 'size' => 4, 'placeholder' => '例)1234', 'maxlength' => '4', 'readonly' => $telReadOnly)); ?>
...
<?php echo Fuel\Core\Form::input('phone_number2_3', isset($_arr_phone2['2']) ? $_arr_phone2['2'] : '', array('class' => 'form-control', 'size' => 4, 'placeholder' => '例)1234', 'maxlength' => '4')); ?>
	...
	<?php }?>
			...
			<tr>
				<th class="text-right">
					こだわり選択
					<div class="text-info" >※*マークは16件まで</div>
				</th>
				<td class="trouble">
					<?php foreach (Trouble::$categories as $k => $v) {?>
						<div class="panel panel-default">
							...
							<div class="panel-body">
								<div class="row">
									<?php foreach (Trouble::$trouble as $key => $value) {
										if ($k == $value['category']) {?>
											<div class="col-md-3">
												<label class="checkbox-inline">
													<?php
													echo \Fuel\Core\Form::checkbox('trouble['.$value['id'].']', $value['id'], substr_count($trouble, ',' . $value['id'] . ',') ? true : false, array('class' => 'trouble'));
													if (Fuel\Core\Input::get('copy_job_id') && \Fuel\Core\Input::get('job_id', 0) == 0) {
														echo $value['adminname'];
													} else {
														if (isset($is_show_old['trouble'])) {
															if (substr_count($old_data['trouble'], ',' . $value['id'] . ',') && substr_count($trouble, ',' . $value['id'] . ',')) {
																echo $value['adminname'];
															} else {
																if (substr_count($trouble, ',' . $value['id'] . ',') || substr_count($old_data['trouble'], ',' . $value['id'] . ','))
																	echo '<span class="edit-before check_box">' . $value['adminname']. '</span>';
																else
																	echo $value['adminname'];
															}
														} else
													...
													if ($value['webtokuname']) {
														echo '<span class="for-recruit">*</span>';
													} ?>
												</label>
											</div>
										<?php }
				...
				<form mehod="get">
					<div class="form-group">
						...
						<div class="form-inline">
							<input id="modal_keyword_search" class="form-control" placeholder="法人名/支店名/SS名/SS住所" type="text" size="100">
                            <button id="modal_button_search" type="button" class="btn btn-primary btn-sm">
                                <i class="glyphicon glyphicon-search icon-white"></i>
                            </button>
						</div>
...
$("#form_ss_id").change(function(){
	...
	$(function() {
		...
		$(document).on('click', '#ssfinder .list-group span.list-group-item', function() {
            ...
            $('#ss-name').attr('addr1', $(this).attr('addr1'))
                .attr('addr2', $(this).attr('addr2'))
                .attr('addr3', $(this).attr('addr3'));
            $('input[name=addr_is_view]:checked').trigger('change');
            $('#hidden_ss_id').val(ss_id);
			$.ajax({
		...
		$('input[name=addr_is_view]').on('change', function() {
		    if ($(this).val() == 0) {
                $('input[name=work_location]').val(
                    $('#ss-name').attr('addr1') + $('#ss-name').attr('addr2') + $('#ss-name').attr('addr3')
                );
            } else {
                $('input[name=work_location]').val(
                    $('#ss-name').attr('addr1') + $('#ss-name').attr('addr2')
                );
            }
        });

        $('#modal_button_search').on('click', function() {
            var keyword = $('#modal_keyword_search').val();
			$.ajax({
				...
				$.each(data, function(key, value) {
				    var item = $('<span class="list-group-item"></span>')
				        .attr('ss-id', value.ss_id)
                        .attr('addr1', value.addr1name)
                        .attr('addr2', value.addr2)
                        .attr('addr3', value.addr3)
                        .text(value.name + ' ' + value.branch_name +  ' ' + value.ss_name);
                    $('.modal_list_ss').append(item);
				});
		...
		$('input[name=category]').on('click', function() {
			$(this).parents('div.panel:first')
				.find('div.panel-body input:checkbox')
				.prop('checked', $(this).prop('checked'));
		});

        var occpations = <?php echo json_encode(Occupation::$rows) ?>;

        $('select[name=occupation_category]').on('change', function() {
            $('select[name=occupation] option[value!=""]').remove();
            var category = $(this).val();
            for (var index in occpations) {
                var occpation = occpations[index];
                if (category == occpation['category']) {
                    var opt = $('<option></option>');
                    opt.attr('value', index).text(occpation['name']);
