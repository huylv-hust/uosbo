...
class Controller_Result extends \Controller_Uosbo
{
	...
	private function _get_data_ss()
	{
		...
		foreach ($list as $ss)
		{
			...
			$result[$ss['ss_id']]['user_id'] = $ss['user_id'];
		}
	...
	private function _count_person($filters,$type = 'order')
	{
		...
		{
			if($person['review_result'] == 1) $count_review_result_1++;
		...
		return array(
			'count_review_result_1'   => $count_review_result_1,
	 ...
	 * @param string $type
	 */
	private function _array_person($filters)
	{
		$result = array();
	 ...
	 * @return array
	 */
	private function _create_data_order($filters,$orders,$all_ss,$all_partner,$all_group,$all_user,$all_sssale)
	{
		$data = array();
		foreach($orders as $order)
		{
			...
			if($order['post_id'] != '')
			{
				...
				if($media->is_web_reprint == 1)
					$double = 2;
				$post_price = $post->price == '' ? 0 : $post->price;
			...
			foreach($arr_ss as $k => $v) {
                ...
                $_personFilter = ['order_id' => $order['order_id'], 'ss_id' => $filters['ss_id']];
                if ($k > $count_ss_list) {
                    $filters['reprinted_via'] = 1;
                    $_personFilter['reprinted_via'] = 1;
                } else {
                    $filters['reprinted_via'] = null;
                    $_personFilter['reprinted_via'] = null;
                }

                $filters['person_id'] = [];
                $persons = $this->m_person->get_filter_person($_personFilter);
                foreach ($persons as $person) {
                ...
                $rpCount = $this->m_person->count_data(['array_person' => $filters['person_id'], 'reprinted_via' => 1]);
				$tmp[3] = ($k > $count_ss_list) ? ($rpCount ? '○' : '') : '';
				$tmp[4] = '';
				$group_name = '';
				$branch_name = '';
				$ss_name = '';
				...
				if(isset($all_ss[$v]) && $all_partner[$all_ss[$v]['partner_code']]['user_id'] != '')
				{
					...
					if(isset($all_ss[$v]['user_id']) && isset($all_user[$all_ss[$v]['user_id']]))
					{
						$username = $all_user[$all_ss[$v]['user_id']]['name'];
					}
					elseif(isset($all_user[$all_partner[$all_ss[$v]['partner_code']]['user_id']]))
					{
				...
				{
					$media_type_text = \Constants::$media_type[ $media->type ];
					$media_budget_type_text = \Constants::$media_budget_type[ $media->budget_type ];
					$media_classification_text = \Constants::$media_classification[ $media->classification ];
					$is_web_reprint_text = \Constants::$is_web_reprint[ $media->is_web_reprint ];
					$media_id = $all_post[$order['post_id']]['m_media_id'];
					$media_name = $media->media_name  .  $web_post;
					$media_version_name = $media->media_version_name;
					$post_name = $all_post[$order['post_id']]['name'];
                    $groupId = $all_partner[ $media->partner_code ]['m_group_id'];
					$branch_name = $all_group[$groupId]['name'] . ' ' . $all_partner[ $media->partner_code ]['branch_name'];
				}
				...
				$tmp[27] = $count_person['count_review_result_1'];
	...
	private function _create_data_job($filters,$jobs,$all_ss,$all_partner,$all_group,$all_user,$all_sssale)
	{
		...
		foreach($jobs as $job)
		{
			...
			$tmp[1] = '';
			$tmp[2] = '';
			$tmp[3] = $job['reprinted_via'] ? '○' : '';
			$tmp[4] = $job['job_id'];
			$tmp[5] = $job['group_name'];
			$tmp[6] = $job['branch_name'];
			$tmp[7] = $job['ss_name'];
			$department_text = '';
			$department_id = '';
			if(isset(\Constants::$department[$job['department_id']]))
			{
				$department_text = \Constants::$department[$job['department_id']];
			...
			$tmp[10] = $username;
			$tmp[11] = '';
			$tmp[12] = \Constants::$sale_type[$job['sale_type']];
			$tmp[13] = '';
			$tmp[14] = '';
			$tmp[15] = '';
			$tmp[16] = '';
			...
			$tmp[24] = '';
			$tmp[25] = '';
			$filters['person_id'] = $job['persons'];
			$tmp[26] = count($job['persons']);
	...
	private function _create_data_person($filters,$persons,$all_ss,$all_partner,$all_group,$all_user,$all_sssale)
	{
		$data = array();
        ...
        $groups = [];
        foreach($persons as $person) {
            $key = $person['ss_id'] . '-' . $person['m_media_id'];
            if (isset($groups[$key]) == false) {
                $groups[$key] = $person;
                $groups[$key]['persons'] = [];
                $groups[$key]['reprinted_via'] = '';
            }
            $groups[$key]['persons'][] = $person['person_id'];
            if ($person['reprinted_via']) {
                $groups[$key]['reprinted_via'] = '〇';
            }
        }

		foreach($groups as $person)
		{
		    $media = \Model_Mmedia::find_by_pk($person['m_media_id']);

			$filters['m_media_id'] = $person['m_media_id'];
			$tmp[0] = 'その他';
			$tmp[1] = '';
			$tmp[2] = '';
			$tmp[3] = $person['reprinted_via'];
			$tmp[4] = '';
			...
			$tmp[13] = '';
			$tmp[14] = '';
			$tmp[15] = $person['post_id'] != '' ? \Constants::$media_type[ $media->type ] : '';
			$tmp[16] = $person['post_id'] != '' ? \Constants::$media_budget_type[ $media->budget_type ] : '';
			$tmp[17] = $person['post_id'] != '' ? \Constants::$media_classification[ $media->classification ] : '';
			$tmp[18] = $person['post_id'] != '' ? \Constants::$is_web_reprint[ $media->is_web_reprint ] : '';
			$tmp[19] = $person['post_id'] != '' ? $person['m_media_id'] : '';
			$tmp[20] = $person['post_id'] != '' ? $media->media_name : '';
			$tmp[21] = $person['post_id'] != '' ? $media->media_version_name : '';
			$tmp[22] = '';
            $tmp[23] = '';
            if ($person['post_id']) {
                $_partner = $all_partner[ $media->partner_code ];
                $tmp[23] = $all_group[$_partner['m_group_id']]['name'] . ' ' . $_partner['branch_name'];
            }
			$tmp[24] = '';
			$tmp[25] = '';
			$tmp[26] = count($person['persons']);
			$count_person = $this->_count_person(['person_id' => $person['persons']], $this->type[2]);
			$tmp[27] = $count_person['count_review_result_1'];
			$tmp[28] = $count_person['count_contact_result_2'];
			$tmp[29] = $count_person['count_review_result_3'];
			$tmp[30] = $count_person['count_review_result_2'];
	...
	public function action_index()
	{
		...
		if(isset($filters['export']) && $filters['export'])
		{
            ...
            $orders = \Model_Orders::filter($filters, null, null);
			$data_export_orders = $this->_create_data_order($filters,$orders,$all_ss,$all_partner,$all_group,$all_user,$all_sssale);

			$jobs = $this->m_job->get_job_for_result_csv($filters);
			$data_export_jobs = $this->_create_data_job($filters,$jobs,$all_ss,$all_partner,$all_group,$all_user,$all_sssale);

			$persons = $this->m_person->get_person_for_result_csv($filters);
			$data_export_persons = $this->_create_data_person($filters,$persons,$all_ss,$all_partner,$all_group,$all_user,$all_sssale);
