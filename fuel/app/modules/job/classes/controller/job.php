...
class Controller_Job extends \Controller_Uosbo
{
	...
	private function _compare_other_data_json($edit_data, $json_data)
	{
		...
		if(isset($edit_data['is_pickup']) && ! isset($data['is_pickup']) && $edit_data['is_pickup'])
			$_arr_key_other['is_pickup'] = true;

		if(isset($edit_data['is_conscription']) && ! isset($data['is_conscription']) && $edit_data['is_conscription'])
	...
	public function action_index()
	{
		...
		if(\Fuel\Core\Input::method() == 'POST')
		{
		    ...
		    if ($job_id == null) {
                $data['post_company_name'] = "株式会社ユーオーエス";
                $data['url_home_page'] = "http://uos.co.jp/";
		        $data['business_description'] = "一般労働者派遣業(許可番号:派23-300159)\n有料職業紹介業(許可番号:23-ユ-300053)";
                $data['work_location_display_type'] = 2;
            }
        }
		...
		$data['sssale_edit'] = array();
        $data['addr1'] = $data['addr2'] = $data['addr3'] = null;
		if($job_id || $copy_job_id)
		{
			...
			$sssale_edit = $sssale_obj->get_sssale_ss($data['ss_id']);
			...
			//get sssale old name
			$sssale_old = \Model_Sssale::find_by_pk($data['old_data']['sssale_id']);
			$data['sssale_name_old'] = \Constants::$sale_type[$sssale_old->sale_type].' '.$sssale_old->sale_name;
			//get ss name old (groupname branchname ssname)
			$ss_edit = $ss_obj->get_list_all_ss(array('ss_id' => $data['ss_id']));
			$data['ss_name_edit'] = ! empty($ss_edit) ? $ss_edit[0]['name'].' '.$ss_edit[0]['branch_name'].' '.$ss_edit[0]['ss_name'] : '';
            if ($ss_edit) {
                $data['addr1'] = \Constants::$address_1[$ss_edit[0]['addr1']];
                $data['addr2'] = $ss_edit[0]['addr2'];
                $data['addr3'] = $ss_edit[0]['addr3'];
            }
			$ss_old = $ss_obj->get_list_all_ss(array('ss_id' => $data['old_data']['ss_id']));
			$data['ss_name_old'] = ! empty($ss_edit) ? $ss_old[0]['name'].' '.$ss_old[0]['branch_name'].' '.$ss_old[0]['ss_name'] : '';
            if ($data['addr1'] == null) {
                $data['addr1'] = \Constants::$address_1[$ss_old[0]['addr1']];
                $data['addr2'] = $ss_old[0]['addr2'];
                $data['addr3'] = $ss_old[0]['addr3'];
            }
		}

        $data['login_info'] = Session::get('login_info');

		$data['occupation_category'] = \Occupation::categoryCode($data['occupation']);
        $data['occupations'] = \Occupation::find($data['occupation_category']);
