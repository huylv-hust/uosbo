...
class Controller_Plan extends \Controller_Uosbo
{
	...
	public function get_webtoku()
	{
		$model = new \Model_Webtokuplan();

		$data = [];

		$data['plans'] = null;
		if (Input::get('start_date') && Input::get('department_id')) {
			$data['plans'] = $model->get_plans(Input::get('department_id'), Input::get('start_date'));
		}

		$data['departments'] = \Constants::$department;
		$data['start_date'] = Input::get('start_date');

		$this->template->title = 'UOS求人システム';
		$this->template->content = \View::forge('plan/webtoku', $data);
	}

	public function post_webtoku()
	{
		$model = new \Model_Webtokuplan();
		...
		if ($model->save_plan(
			Input::post('department_id'),
			Input::post('start_date'),
			Input::post('plans')
		))
		{
			$return = 'success';
		...
		$uri = 'job/plan/webtoku?' . Input::server('QUERY_STRING');
