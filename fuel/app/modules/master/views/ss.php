...
<table class="table table-striped">
	...
	<tr>
		<th class="text-right">取引先勤務地名</th>
		<td>
	...
	<tr>
		...
		<td>
			<?php echo Form::input('tel_1', Input::post('tel_1') ? Input::post('tel_1') : (isset($json->tel) ? substr_count($json->tel, '-') ? explode('-', $json->tel)[0] : substr($json->tel, 0,4) : ''), array('class' => 'form-control', 'size' => 5, 'maxlength' => 5));?>
            -
    ...
    <tr>
        <th class="text-right">位置情報</th>
        <td>
            <div class="input-group">
                <div class="input-group-addon">緯度</div>
                <?php echo Form::input('lat', Input::post('lat', isset($json->lat) ? $json->lat : ''), array('class' => 'form-control', 'size' => 30));?>
            </div>
            <label id="form_lat-error" class="error" for="form_lat"></label>
            <div class="input-group">
                <div class="input-group-addon">経度</div>
                <?php echo Form::input('lon', Input::post('lon', isset($json->lon) ? $json->lon : ''), array('class' => 'form-control', 'size' => 30));?>
            </div>
            <label id="form_lon-error" class="error" for="form_lon"></label>
            <p></p>
            <div class="edit-before <?php echo isset($is_view['lat']) || isset($is_view['lon']) ? '' : 'hide'?>">
                <div class="input-group <?php echo isset($is_view['lat']) ? '' : 'hide'?>">
                    <div class="input-group-addon">緯度</div>
                    <input class="form-control" type="text" disabled value="<?php echo isset($ss) ? $ss->lat : '' ?>">
                </div>
                <div class="input-group <?php echo isset($is_view['lon']) ? '' : 'hide'?>">
                    <div class="input-group-addon">経度</div>
                    <input class="form-control" type="text" disabled value="<?php echo isset($ss) ? $ss->lon : '' ?>">
                </div>
            </div>
        </td>
    </tr>
	...
	<tr>
		...
		<td>
            <div class="input-group">
                <div class="input-group-addon">UOS担当者営業所</div>
			<?php
			...
			$arr_user = array();
			$department_id = '';
            if(isset($json->user_id) && $json->user_id) {
                $user_id = $json->user_id;
                $user = Model_Muser::find_by_pk($json->user_id);
                $department_id = $user->department_id;
                $arr_user_temp = Model_Muser::find(['department_id' => $department_id]);
                foreach ($arr_user_temp as $row) {
                    $arr_user[$row['user_id']] = $row['name'];
                }
            }
            ...
            <div class="input-group">
                ...
                <?php echo Fuel\Core\Form::select('user_id',$user_id,array(''=>'担当者を選択してください')+$arr_user,array('class' => 'form-control'));?>

            </div>

			<label id="form_user_id-error" class="error" for="form_user_id"></label>
			<div class="edit-before <?php echo isset($is_view['user_id']) ? $is_view['user_id'] : 'hide'?>">
				<?php
				if(isset($is_view['user_id']))
				{
                    $department_id = '';
                    $user_id= '';
                    $arr_user = array();
                    if(isset($ss->user_id) && $ss->user_id)
                    {
                        $user_id = $ss->user_id;
                        $user = Model_Muser::find_by_pk($user_id);
                        $department_id = $user->department_id;
                        $arr_user_temp = Model_Muser::find(['department_id' => $department_id]);
                        foreach($arr_user_temp as $row){
                            $arr_user[$row['user_id']] = $row['name'];
