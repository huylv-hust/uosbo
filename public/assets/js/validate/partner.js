var partners = function () {
    ...
    //open modal search group
    var open_modal_group = function () {
        $('button[name=findgroup-btn]').on('click', function () {
            $('#groupfinder').modal();
            return false;
        });
    };
    ...
    //search group
    var search_group = function () {
        $('#btn_group_search').on('click', function () {
            var keyword = $('#groupfinder .modal_keyword').val();
            $.ajax({
                url: baseUrl + 'ajax/common/searchGroup',
                method: 'POST',
                data: {keyword: keyword},
                dataType: 'json'
            }).success(function (data) {
                var html = '<span class="list-group-item disabled">検索結果</span>';
                $.each(data, function (key, value) {
                    html += '<a href="javascript:void(0)" class="list-group-item" group-id="' + value.m_group_id + '">' + value.name + '</a>';
                });

                $('#groupfinder .list-group').html(html);
                if (data.length == 0) {
                    alert('見つかりませんでした');
                }
            });
    ...
    //choose group
    var choose_group = function() {
        $('#groupfinder').on('click', 'a.list-group-item', function() {
            $('#group-name').text($(this).text());
            $('[name=m_group_id]').val($(this).attr('group-id'));
            $('#groupfinder').modal('hide');
            return false;
        });
    };
